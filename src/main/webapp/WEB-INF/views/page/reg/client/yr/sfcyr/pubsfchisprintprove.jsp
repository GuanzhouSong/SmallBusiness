...
<div class="mod info-title">
    <h5 class="pdt20 center">农民专业合作社年度报告证明</h5>
	            ...
	            <table class="table-public" border="0" cellspacing="0" cellpadding="0">
				   ...
				   <tr>
					...
					<tr>
	                    ...
	                    <c:choose>
							<c:when test="${midBaseInfoDto.regState == 'D' || midBaseInfoDto.regState == 'DA' }">
								<td width="30%">吊销</td>
							</c:when>
							<c:when test="${midBaseInfoDto.regState == 'C'}">
								<td width="30%">撤销</td>
							</c:when>
							<c:when test="${midBaseInfoDto.regState == 'XX' || midBaseInfoDto.regState == 'DX'}">
								<td width="30%">注销</td>
							</c:when>
							<c:when test="${midBaseInfoDto.regState == 'QX'}">
								<td width="30%">迁出</td>
							</c:when>
							<c:otherwise>
								<td width="30%">存续</td>
        ...
        <div class="content content-contact">
            <div style="overflow-y: auto;">
                <table class="table-public table-web" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tbody>
                    <c:if test="${empty sfcBaseInfoHisList}">
		        		<tr>
		        			<td colspan="2" style="text-align:center;">无</td>
		        		</tr>
		        	</c:if>
		        	<c:if test="${not empty sfcBaseInfoHisList}">
		        		<c:forEach var="sfcBaseInfoHis" items="${sfcBaseInfoHisList}" varStatus="status">
			        		<tr>
			        			<td >${status.count}</td>
			        			<td >
			        				<fmt:formatDate value="${sfcBaseInfoHis.lastReportTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
			        			</td>
			        		</tr>
		        		</c:forEach>
		        	</c:if>
...
<script src="<c:url value="/js/reg/client/yr/pubprint/edit_main.js"/>"></script>
<script>
    window._CONFIG = {
       _year:'${year}', //年报年度
        _pripid:'${pripid}'  //主体代码 
