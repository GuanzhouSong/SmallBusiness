	...
	function showgd(){
		...
		http.httpRequest({
			...
			success:function(json){
				...
				$("#js_baseinfoCount").text(json.data.baseinfocount == null ? 0 : json.data.baseinfocount);
				$("#js_entAboveCount").text(json.data.entAbovescalecount == null ? 0 :json.data.entAbovescalecount);
				$("#js_dcdyCount").text(json.data.mortRegInfocount == null ? 0 : json.data.mortRegInfocount);
				$("#js_gjsbCount").text(json.data.tmDetailInfocount == null ? 0 : json.data.tmDetailInfocount);
				$("#js_bdwycount").text(json.data.eightIndutryCount == null ? 0 : json.data.eightIndutryCount);
			}
		});
		//查询小微年报相关统计
		http.httpRequest({
			type:'POST',
			url:'/sment/entarchives/smbaseinfoNbCount.json',
			success:function(json){
				$data = json.data;
				if(!$data){
					return;
				}
				$("#js_addProCount").text($data.addProCount == null ? 0 : $data.addProCount);
				$("#js_ratGro").text($data.ratGro == null ? 0 :$data.ratGro);
				$("#js_empNum").text($data.empNum == null ? 0 : $data.empNum);
			}
		});
		//查询三次产业比例
		http.httpRequest({
			type:'POST',
			url:'/sment/entarchives/smbaseThreeTypeCount.json',
			success:function(json){
				$data = json.data;
				if(!$data){
					return;
...
//				        },
	                    color: ['#5B9CD5','#ED7D31', '#9e9e9e'],//设置显示的颜色
	                    xAxis : [
	                             {
	                                 type : 'category',
			...
			success:function(json){
				...
				var option = {
					    ...
					    legend: {
							...
							itemHeight:10,
							selectedMode:false,
					        data:['特色小镇','众创空间','科技孵化园','跨境电商园','其他','未选择']
					    },
	                    color: ['#5B9CD5','#ED7D31','#999999','#fff800','#3399ff','#70AD46'],//默认显示的颜色
					    series: [
					        {
					            name:'区域分类',
					            type:'pie',
					            ...
					            data:[
					                ...
					                {value:$entarchivesDto.kjdsyCount, name:'跨境电商园'},
					                {value:$entarchivesDto.qtCount, name:'其他'},
					                {value:$entarchivesDto.wxzCount, name:'未选择'}
					            ]
					        }
					    ]
			};
...
//						},
						...
						legend: {
							...
							data:['信息经济产业','环保产业','旅游产业','金融产业','健康产业','时尚产业','高端装备制造产业','文化创意产业','未选择']
						},
...
//					                }
						        	 ...
						        	 data:[
						        	       ...
						        	       {value:$entarchivesDto.industryType9, name:'未选择'}
						        	       ]
						         }
						         ]
				};
			...
			success:function(json){
				...
				var option = {
						...
						legend: {
							...
							itemHeight:10,
							selectedMode:false,//图例选择不开启
							data:['毕业三年内大学生','高校科研院所专业技术人才','海外归国人才','返乡农民工','企业员工辞职创业','退役士兵','其他','未选择']
						},
						color: ['#5B9CD5','#ED7D31','#999999','#fff800','#3399ff','#70AD46','#13598a','#c23531'],//默认显示的颜色
						series: [
						         {
						        	 name:'培育库企业类型',
						        	 type:'pie',
						        	 ...
						        	 data:[
						        	       ...
						        	       {value:$entarchivesDto.entCrowdCode8, name:'未选择'}
						        	       ]
						         }
						         ]
				};
...
//				myChartadd_last.setOption(option); 
			}});
	}
					    ...
					    series: [
					        {
					            ...
					            label: {
					                normal: {
					                    show: false,
//					                    position: 'outside',
//					                    formatter:'{b}:\n {c}户'//模板变量有 {a}、{b}、{c}、{d}，分别表示系列名，数据名，数据值，百分比。
					                },
//					                emphasis: {
//					                    show: true,
//					                    textStyle: {
...
//        	mychartadd_first.setOption(option3_1);
