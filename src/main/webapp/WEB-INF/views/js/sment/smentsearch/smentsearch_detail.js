require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate','pagination'], function (layer, dataTable, util, http,handlebars) {
	...
	table_yzwfsxxx,table_qyzlxx,table_qybdccfxx,table_qybdcdyxx,table_forguaranteeinfo,table_pyxx;
    ...
    /* 企业年报信息*/
    function init_qynbxx(){
    	loadData_yrinfo();
    }
    
    /* 工商业务信息*/
    function init_gsywxx(){
    	initPubotherlicenceTable();//行政许可信息
    	initDataTable_qysbxx();//企业商标信息
    	initDataTable_qyzlxx();//企业专利信息
    	initDataTable_qydcdyxx();//动产抵押信息
    	initDataTable_qybdcdyxx();//不动产抵押信息
    	initDataTable_zscqczdjxx();//知识产权出质登记信息
    	initDataTable_qygqczxx();//股权出质登记信息
    	initDataTable_forguaranteeinfo();//对外提供担保信息
    	initDataTable_ccjcxx();//抽查检查信息
    	initDataTable_qyjyqcxx();//经营异常名录
    	initDataTable_yzwfsxxx();//严重违法失信
    	initDataTable_pubOtherpunish();//行政处罚信息
    	initDataTable_qygqdjxx();//司法协助信息
    	initDataTable_qybdccfxx();//不动产查封信息
    }
    
    /* 企业培育信息*/
    function init_qypyxx(){
    	if((table_pyxx+'')=='undefined'){
    		initDataTable_pyxx();
    	}
    }
    ...
    function loadData_yrinfo() {
        ...
        if(entTypeCatg==50){//个体户
        	...
        	url = "/sment/smBaseinfo/yrinfolist.json";
        }
        http.httpRequest({
            url: url,
            serializable: false,
            ...
            success: function (data) { 
	        	if(data.data==""||data.data==null){//没有数据
	             	$('#pubtable-yrinfo').html("<td valign='top' colspan='6' class='dataTables_empty'>无</td>"); 
	             	$("#yrinfoTotal").text(0); 
	             	//没有数据时隐藏表格
                ...
                handlebars.registerHelper("transIsSment",
                        function(value) {
    	                	if(value){
    	                		if(value=='1'){
    	                			return "是";
                ...
                if(typeof itemCount!='undefined' &&itemCount>0){
                	$('#table-yrinfo').html(myTemplate(data));
                }
            }
        });
    }
    ...
    function initDataTable_qyzlxx(){
    	...
    	table_qyzlxx.on('xhr.dt',function(e,settings,json,xhr){
    		if(json.recordsTotal <= 5){
    			$('#tab_qyzlxx .dataTables_wrapper .bottom').hide();
    		}else{
    			$('#tab_qyzlxx .dataTables_wrapper .bottom').show();
    		}
        })
    }
...
//              {data: 'country',className: 'center'},
    ...
    //企业培育信息
    function initDataTable_pyxx(){
    	http.httpRequest({
        	type:"POST",
        	url:'/sment/entarchives/help/doGetEntarchivesHelpListJSON',
        	data :{"priPID":$("#priPID").val()},
        	success:function(data){
        		$("#pyxx").html(data.msg);
        		var map= data.data;
        		var str='<div id="table_pyxx" style="display:none">';
				if(data.msg=='0'){
					str=str+'<table  class="table-row" border="0" cellspacing="0" cellpadding="0" ><tr><th>序号</th><th>需求类别</th><th>需求内容</th><th>帮扶时间</th><th>帮扶内容</th></tr>';	
					str=str+'<tr><td colspan="5">无</td></tr></table>';
				}else{
	        		for(var key in map){
	        			str=str+'<table  class="table-row" border="0" cellspacing="0" cellpadding="0" ><tr><td colspan="5" style="text-align:left;color:black;font-size:15px;border:none"><b>'+key+'</b></td></tr><tr><th>序号</th><th>需求类别</th><th>需求内容</th><th>帮扶时间</th><th>帮扶内容</th></tr>';
	        			for(var i=0;i<map[key].length;i++){
	        				var obj=map[key][i];
	        				if(obj!=null||obj!=""){
	        				str=str+'<tr><td>'+(i+1)+'</td><td>'+obj.helpTypeName+'</td><td>'+obj.helpText+'</td><td>'+obj.helpTime+'</td><td>'+obj.helpContent+'</td></tr>';
	        				}
	        			}
	        			str=str+'</table>';
	        		}
				}
        		$("#tab_pyxx").append(str+'</div>');
        	}
        });
    }
    ...
    function bind(){
        ...
        {//查看年报信息
            ...
            handler: function () {
                 ...
                 var currency =window._CONFIG._currencyCn;
                 var classFlag = '1';
                 //window.open('http://zj.gsxt.gov.cn/entinfo/yrinfo?year=' + year + "&encryPriPID=" + encryPriPID + "&classFlag=" + classFlag, 'newwindow');
                 layer.dialog({
						title:'小微企业年报公示信息',
						area: ['100%', '100%'],
						content: '/sment/smBaseinfo/smyr?year='+year+"&encryPriPID="+encryPriPID+"&currency="+currency,
        ...
        },
        {
        	...
        	event:'click',
        	handler:function(){
        		initAddflex("archiveInfo","table_pyxx");
        	}
        }
        ])
    }
