require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate','jquery.multiselect','jquery.formautofill','ztree','ztreeCheck','ztreeHide'], function (layer, dataTable, util, http,handlebars) {
    ...
    function getEntinfo(priPID){
    	http.httpRequest({
            url: '/sment/entarchives/queryEntInfoByPriPID.json',
            data:{pripid:priPID},
            type: 'post',
            success: function(data) {
    ...
    function bind() {
    	...
    	util.bindEvents([{
        	...
        	el:'#tsxzName,#zckjName,#kjfhyName,#kjdsyName,#qtName',
        	event:'click',
        	handler:function(){
	        	var thisObj = $(this);
	        	showMenu(thisObj);
        	...
        	el:'#a_tsxzName,#a_zckjName,#a_kjfhyName,#a_kjdsyName,#a_qtName',
        	event:'click',
        	handler:function(){
        		var thisObj = $(this);
