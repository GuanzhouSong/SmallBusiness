require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate','pagination'], function (layer, dataTable, util, http,handlebars) {
    ...
    function init_djxkxx(){
    	if((table_ggryxx+'')=='undefined')
    		initDataTable_ggryxx(); // 高管
    	if((table_fzjgxx+'')=='undefined'){
    		initDataTable_fzjgxx(); // 分支
         }
    	if((table_qsxx+'')=='undefined'){
    		initDataTable_qsxx();  
    	}
    	if((table_bgxx+'')=='undefined'){
    		initDataTable_bgxx();
    	}
    }
    ...
    //即时股东及出资信息   
    function initDataInvestmentTable(page,length,callback) { 
        http.httpRequest({
           ...
           // data:  {start: 0,length: 9999,params:params},
            ...
            success: function (data) {
                ...
                $('#investment_pagination_one').pagination(showPage, {
                    prev_text: '<i class="icon-triangle-left"></i>上一页',
                    next_text: '下一页<i class="icon-triangle-right"></i>',
                    num_display_entries: 3,//主体页数
                    num_edge_entries: 2,//边缘页数
                    ...
                    last_text: '末页',
                    callback:function(page,jq){
                    	initDataInvestmentTable(page,5);
                    }
                });
                var handleHelper = handlebars.registerHelper("addOne", function(index) {return index+1+(page*5);});//序号
                handlebars.registerHelper("trsPubFlag", 
                    ...
                    $("#investmenitemCount").text(showPage);//总数
                    var pageNum = showPage%5==0?showPage/5:Math.ceil(showPage/5);
                    $("#investmenpageCount").text(pageNum);//共多少页
             }
        })
    }
    	...
    	http.httpRequest({
            ...
            success: function (data) {
            	if(data.liqMem != null){
                	...
                	$("#clearInfo").addClass("flexed-icon");
...
//              {data: 'country',className: 'center'},
    ...
    function bind(){
        ...
        },
		{
			...
			event: 'click',
			handler: function () {
				var priPID =window._CONFIG._encryPriPID;
				window.open("/reg/server/rptyr/relationnetall/relationShow?priPID="+priPID,'newwindow');
			}
		},
        ...
        },
        {
            ...
            handler: function () {
            	var encryPriPID =window._CONFIG._encryPriPID;
            	if(encryPriPID!=null && encryPriPID != ''){
            		window.open('/syn/panoramasearch/doEntAppCzlcSearchDetails/' + encryPriPID,'_blank');
            	}
            }
        }])
    }
  ...
  //翻页事件 
    ...
    function tabSwitch(n){
    	...
    	switch(n){
	    	...
	    	case 3:
	    		...
	    		break;
	    	case 4:
	    		initDataTable_qydcdyxx();
	    		initDataTable_qybdcdyxx();
	    		initDataTable_qygqczxx();
	    		initDataTable_zscqczdjxx();
	    		initDataTable_forguaranteeinfo();
	    		break;
	    	case 5:
	    		init_ccjcxx();break;
	    	case 6:
	    	...
	    	case 8:
	    		loadData_yrinfo() ;
	    		loadData_er_esc_appinfo();
	    		initDataInvestmentTable(0,5);
	    		//initDataInvsraTable();
	    		initDataTable_gqbgxx();
	    		//initPermitTable();
	    		initDataTable_jsxzxkxx();
	    		//initIppldgTable();
	    		initCaseTable();
	    		//initDataTable_zscqczdjxx();
	    		/*init_dsfxx();*/break;
    ...
    function getTabTotal() { 
        http.httpRequest({
            url: '/syn/panoramasearch/getTabTotal',
            serializable: false,
            data: {start: 0,length: 1,params:params},
            type: 'post',
            success: function (data) {
            	//备案登记tab
                var badjCount = data.data.badjCount == null ? 0 : data.data.badjCount; 
            	//出资投资
                var cztzCount = data.data.cztzCount == null ? 0 : data.data.cztzCount;
                //许可资质
                var xkzzCount = data.data.xkzzCount == null ? 0 : data.data.xkzzCount; 
                //知识产权tab
                var zscqCount = data.data.zscqCount == null ? 0 : data.data.zscqCount; 
            	//抵押质押担保
                var dyzydbCount = data.data.dyzydbCount == null ? 0 : data.data.dyzydbCount;
                //抽查检查
                var cxjcCount = data.data.cxjcCount == null ? 0 : data.data.cxjcCount; 
                //不良信息
                var blxxCount = data.data.blxxCount == null ? 0 : data.data.blxxCount;
                //良好荣誉
                var lhryCount = data.data.lhryCount == null ? 0 : data.data.lhryCount;
                //企业公示
                var qygsCount = data.data.qygsCount == null ? 0 : data.data.qygsCount;
                ...
                http.httpRequest({
                    ...
                    success: function (data) {
        	        	if(data.data!=""||data.data!=null){//没有数据
        	        		qygsCount = qygsCount+data.recordsTotal; //获取查询出来的总数
        	            }  
                    }
                }); 
            	...
            	http.httpRequest({
                    ...
                    success: function (data) { 
                    	if(data.liqMem != null){
                    		badjCount=badjCount+1;
                    	 } 
                      }
                }); 
                $("#badjCount").text("("+badjCount+")");
                if(badjCount>0){
                	 $("#badjCount").css("color","red");
                } 
                $("#cztzCount").text("("+cztzCount+")");
                if(cztzCount>0){
               	 $("#cztzCount").css("color","red");
                }
                $("#xkzzCount").text("("+xkzzCount+")");
                if(xkzzCount>0){
               	 $("#xkzzCount").css("color","red");
                }
                $("#zscqCount").text("("+zscqCount+")");
                if(zscqCount>0){
               	 $("#zscqCount").css("color","red");
                }
                $("#dyzydbCount").text("("+dyzydbCount+")");
                if(dyzydbCount>0){
               	 $("#dyzydbCount").css("color","red");
                }
                $("#cxjcCount").text("("+cxjcCount+")");
                if(cxjcCount>0){
               	 $("#cxjcCount").css("color","red");
                }
                $("#blxxCount").text("("+blxxCount+")");
                if(blxxCount>0){
               	 $("#blxxCount").css("color","red");
                }
                $("#lhryCount").text("("+lhryCount+")");
                if(lhryCount>0){
               	 $("#lhryCount").css("color","red");
                }
                $("#qygsCount").text("("+qygsCount+")");
                if(qygsCount>0){
               	 $("#qygsCount").css("color","red");
                }
            }
        });
    }
