require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'handlebars', 'jquery','jquery.serialize','laydate','jquery.validate'], function (layer, dataTable, util, http, handlebars) {
    ...
    function save(){
		...
		var isAuditState = $("input[name='auditState']").is(':checked');
		if(arr.length<=0){
		...
		}else if(!isAuditState){
			layer.msg("审核结果是必填项", {time: 2000}, function() {});
		}else{
			var searchParams = $("#taskForm").serializeObject();
			http.httpRequest({
				url: '/syn/sccheck/pubscentresult/auditscresult',
				dataType:"json",  
	                  	...
	                  	if(returncodesArr.length>0){
	                  		for(var i=0;i<returncodesArr.length;i++){
	                			 ...
	                			 var trHTML = "<tr>"
		                                	...
		                                	+ ">是"
		                                	+ "</td>"
		                                	+ "<td><textarea class='prodis' style='overflow-y:auto;display: none' id='problem_"+tCode+"' name='problem_"+tCode+"'></textarea></td>"
		                                	+ "<td><textarea class='prodis' style='overflow-y:auto;display: none' id='dispose_"+tCode+"' name='dispose_"+tCode+"'></textarea></td>"
		                                	+ "</tr>"
            ...
            handler: function () {
            	...
            	if(num==1){
            		var trHTML = "<tr>"
            			...
            			+ ">是"
            			+ "</td>"
            			+ "<td><textarea class='prodis' style='overflow-y:auto;display: none' id='problem_OTHER"+num+"' name='problem_OTHER"+num+"'></textarea></td>"
            			+ "<td><textarea class='prodis' style='overflow-y:auto;display: none' id='dispose_OTHER"+num+"' name='dispose_OTHER"+num+"'></textarea></td>"
            			+ "</tr>"
            		 $("#table2").append(trHTML);
            	}else{
