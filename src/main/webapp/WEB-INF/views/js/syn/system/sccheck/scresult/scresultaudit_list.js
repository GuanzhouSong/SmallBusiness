require(['component/iframeLayer', 'component/dataTable', 'common/util', 'common/http', 'jquery','jquery.serialize','laydate','jquery.multiselect'], function (layer, dataTable, util, http) {
    ...
    function initDataTable() {
        ...
        table = dataTable.load({
            ...
            ajax: {
            	type : "POST",
                url:'/syn/sccheck/pubscentresult/auditScresultList.json',
                data:function(d){
            ...
            columnDefs: [
            	{
 					...
 					render:function(data,type,row,meta){
 						if(row.auditState=='2'){
 							return "<a href='javascript:void(0)' class='js_audit'>审核</a>";
 						}else if(row.auditState=='5'&&row.disposeState=='1'){
 							return "<a href='javascript:void(0)' class='js_supply'>续录</a>";
 						}else{
 							return "<a href='javascript:void(0)' class='js_detail'>详情</a>";
 						}
 					}
 				 },{
 					targets:2,
        ...
        util.bindEvents([
	        {
	            ...
	            handler: function () {
	                ...
	                layer.dialog({
	                    title: '',
	                    area: ['95%', '95%'],
	                    content: '/syn/sccheck/pubscentresult/auditscresultpage?taskUid='+data.taskUid+"&priPID="+data.priPID+"&uid="+data.uid,
