 ...
 */
@Service
public class PubDtInfoQuartzJobServiceImpl extends MyBatisServiceSupport implements IPubDtInfoQuartzJobService {
	private static final Logger logger = LoggerFactory.getLogger(PubDtInfoQuartzJobServiceImpl.class);
	...
	private final String exaCodes="183";
		...
		try {
			...
			if("5".equals(flag)||"4".equals(flag)){
				if("5".equals(flag)){
					...
					if(csQuartzJobService.checkJobIsExecute(QuartzJobName.DTINFO_JOB.getJobName())){
						return "任务已执行";
					}
					csQuartzJobService.insert(QuartzJobName.DTINFO_JOB.getJobName());
					logger.info("双告知分流定时任务开始");
				}
			...
			//获取一周内有审批事项的在册核准数据
			List<MidBaseInfo> midBaseInfoList=midBaseInfoService.selectMidInfoByApprDate(parmMap);
			if(midBaseInfoList.size()>0){
				//循环为每一个企业推送消息
				for(MidBaseInfo midBaseInfo:midBaseInfoList){ 
					try { 
						List<PubDtInfo> pubDtInfoPushDataList=new ArrayList<PubDtInfo>();
	                    //单个企业的审批事项 特殊情况下会有多个
						...
						//企业所在行政区划
						String yiedistrict=midBaseInfo.getYiedistrict()==null?"":midBaseInfo.getYiedistrict();
						...
						//获取已经推送过的数据
						List<PubDtInfo> pubDtInfoList= pubDtInfoService.selectDistinctExaCodeByRegNoAndPriPID(regNo, priPID);
							...
							for(String hzApprCode:hzApprCodeArrList){ 
							    ...
							    if(LicDeptArr.length>0){
							    	for(int b=0;b<LicDeptArr.length;b++){
						    			...
						    			//粮食局不下放省局登记或管辖数据
						    			if(!exaCodes.equals(hzApprCode)){
						    				//如果是省直属分局或省局管辖的直接推送到住所地
							    			if(("330010".equals(temOwnership)||"330000".equals(temOwnership))||("330010".equals(localAdmin)||"330000".equals(localAdmin))){
							    				temOwnership=domDistrict;
							    			}
						    			}
...
*/						    			this.doSetDtInfoListVal(hzApprCodeList, pubDtInfo, pubDtInfoPushDataList, midBaseInfo, hzApprCode, exaName, regType, temOwnership, spFlag,depType);
				...
				message="双告知分流定时任务结束:总共推送了【"+totalPushCount+"】条记录";
				logger.info(message);
			}else{
				//message="没有找到核准日期自"+startDate+"至"+endDate+"新增/变更的核准数据双告知分流定时任务结束";
				logger.info(message);
			}
			return message;
		} catch (Exception e) {
			 e.printStackTrace();
			 logger.error("双告知定时任务执行失败", e);
	 ...
	 * @throws
	 */
	public void  doDeleteRepeatedElement(List<String> appConPptArrList,List<PubDtInfo> pubDtInfoList)throws Exception{
		for(int a=0;a<appConPptArrList.size();a++){
			for(PubDtInfo dtInfo:pubDtInfoList){
	...
	/**
	 * 
	 * 描述   根据审批事项编码和部门编码获取是否推送属地标识
	 * @author 赵祥江
	 * @date 2016年10月25日 下午2:23:24 
	 * @param 
	 * @return String
	 * @throws
	 */
	...
	public String getlicDeptTypeByHzApprCodeAndDeptCode(String hzApprCode,String isFlag){
		...
		try { 
			...
			e.printStackTrace();
			logger.error(" 根据审批事项编码获取是否推送标识失败", e);
			return licDeptType;
		} 
		return licDeptType;
	}
	...
	/**
	 * 
	 * 描述   组装双告知主表数据并把数据加入保存的结果集List：如果是审批部门先检索审批部门对照表 如果检索到则审批部门推送成功否则推送失败，
	...
	public void doSetDtInfoListVal(List<String> hzApprCodeList,
			PubDtInfo pubDtInfo, List<PubDtInfo> pubDtInfoPushDataList,
		...
		if("1".equals(spFlag)){
			//根据审批事项、行政区划和职能部门编码
			List<LicDept> licDeptList=licDeptService.selectLicDeptByExaCodeAndDutyDeptCodeAndYiedistrict(exaCode, depType, deptCode,"1");
			if(licDeptList!=null&&licDeptList.size()>0){
				...
				if(sysDepartListTem!=null&&sysDepartListTem.size()>0){
					...
					msg="没有找到"+areaName+"下属的"+dutyDeptName+"与职能部门的对应关系或该部门不存在,审批部门编码【"+deptCode+"】,【职能部门编码:"+depType+",职能部门名称:"+dutyDeptName+"】,请在部门维护功能检查配置或联系系统管理员";
					} 
				...
				//根据编码获取区县名称 
				areaName=getAreaName(deptCode);
				msg="没有找到对应的审批部门【编码:"+deptCode+"】,【职能编码:"+depType+",职能部门名称:"+dutyDeptName+"】,请在审批对照维护功能检查配置或联系系统管理员";
				if(StringUtil.isNotEmpty(areaName)){
					msg="没有找到"+areaName+"下属的【"+dutyDeptName+"】,审批部门编码【"+deptCode+"】,【职能部门编码:"+depType+",职能部门名称:"+dutyDeptName+"】,请在审批对照维护功能检查配置或联系系统管理员";
				}
				pubDtInfo.setReassignRemark(msg);
			    pubDtInfo.setCheckPushType("3"); 
			}  
	 ...
	 * @throws
	 */
	public void setPubDtInfoVal(MidBaseInfo midBaseInfo, PubDtInfo pubDtInfo,
	...
	private  List<SysDepart> checkDepExist(String adcode)throws Exception {
		try {
			...
			e.printStackTrace();
			logger.error(" 根据部门编码和职能编码校验是否配置有部门与职能的关系表失败", e);
		}
		return null;
 	}
   ...
   /**
	...
	public String pubFailPush() throws Exception {
		...
		//获取推送失败的企业数据
		List<PubDtInfoDto> pubDtInfoDtoList= pubDtInfoService .queryPubdtinfoFailReBackListJSON(request);
		if(pubDtInfoDtoList!=null&&pubDtInfoDtoList.size()>0){ 
			for(PubDtInfoDto pubDtInfoDto:pubDtInfoDtoList){
				try {
					...
					PubDtInfo pubDtInfo=new PubDtInfo(); 
					PubDtInfo pubDtInfo1=pubDtInfoService.selectPubDtInfoByUID(uId);
					//后置审批事项编码
					...
					//根据审批事项、行政区划和职能部门编码
					List<LicDept> licDeptList=licDeptService.selectLicDeptByExaCodeAndDutyDeptCodeAndYiedistrict(exaCode, dutyDeptCode, deptCode,"1");
					if(licDeptList!=null&&licDeptList.size()>0){
						//对应审批部门
						String deptCodeTem=licDeptList.get(0).getDeptCode();
						//对应审批部门名称
