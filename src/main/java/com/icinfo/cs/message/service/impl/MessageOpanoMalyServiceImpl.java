				...
				for(String priPID:priPIDList){
					...
					if(pubEppassword!=null){
						...
						if(this.checkTel(tel)){
							  ...
							  String json = "{\"title\" : \"列入经营异常名录提醒\", " + "\"pusher\" : \"国家企业信用信息公示系统\", " + "\"receiver\" : \""+tel+"\"," + "\"sourceUrl\" : \""+url+"/message/opanomalymessage/toMessageOpanoMalyView?msgReadTel="+tel+"&isMove=3&sendDate="+DateUtil.getDate ("yyyyMMdd")+"\"," + "\"type\" : \"3\",   " + "\"subType\" : \"yr_in_manage_error\"," + "\"content\" : \""+content+"\"," + "\"additionInfo\" : {" + "\"remark\" : \"列入经营异常名录是企业的信用污点,请及时消除异常事由，并向工商部门申请移出经营异常名录。\"," + "\"recordCategory\" : \"信用警示\"  " + "}," + "\"pushChannels\" : \"1,2\"   " + "}";
							  //发送消息
				...
				for(String priPID:priPIDList){
					...
					if(pubEppassword!=null){
						...
						if(this.checkTel(tel)){
							  ...
							  String json = "{\"title\" : \""+title+"\", " + "\"pusher\" : \"国家企业信用信息公示系统\", " + "\"receiver\" : \""+tel+"\"," + "\"sourceUrl\" : \""+url+"/message/opanomalymessage/toMessageOpanoMalyView?msgReadTel="+tel+"&isMove="+isMove+"&sendDate="+DateUtil.getDate ("yyyyMMdd")+"\","  + "\"type\" : \"3\",   " + "\"subType\" : \"yr_out_manage_error\"," + "\"content\" : \""+content+"\"," + "\"additionInfo\" : {" + "\"remark\" : \"请持续关注企业信用警示提醒消息。\"," + "\"recordCategory\" : \"信用警示\"  " + "}," + "\"pushChannels\" : \"1,2\""+"}";
							  //发送消息
							  int i=sendPOSTRequestMsg(json,tel);
							  //失败
							  String state="0";
