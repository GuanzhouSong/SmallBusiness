...
import com.icinfo.cs.system.service.impl.DeptDutycodeRefServiceImpl;
...
/**
 * 描述:    cs_pub_dtinfo 对应的Service接口实现类.<br>
 *
 * @author framework generator
 * @date 2016年10月18日
 */
@Service
public class PubDtInfoServiceImpl extends MyBatisServiceSupport implements IPubDtInfoService {

	private static final Logger logger = LoggerFactory.getLogger(PubDtInfoServiceImpl.class);

	@Autowired
	private PubDtInfoMapper pubDtInfoMapper; 
	//职能编码方法
	@Autowired
	 ...
	 */
	@Override
	public List<PubDtInfoDto> queryPubdtinfoFailReBackListJSON(PageRequest request)
			throws Exception {
		try {
	...
	public int updatePubDtInfoByUID(PubDtInfo pubDtInfo) throws Exception {
		try {
			...
			if(pubDtInfo!=null&&StringUtil.isNotEmpty(pubDtInfo.getUid())){
				Example example = new Example(PubDtInfo.class);
		...
		if(map !=null){
			if(map.get("claimFlag") != null){
				if(sysUser.getUserType().equals("1") && StringUtil.isNotEmpty(sysUser.getDeptCode())){ //警示人员登录：
					...
					map.put("checkDep", sysUser.getSysDepart().getAdcode()); //当前用户的行政编码即为审批部门编码
				}
			}
			//查询时条件处理
	 ...
	 */
	@Override
	public List<PubDtInfoDto> selectPubDtInfoCount(PageRequest request)
			throws Exception {
		try {
			Map<String, Object> searchMap=  request.getParams();
	 ...
	 */
	@Override
	public int insertBatchPubDtinfo(List<PubDtInfo> pubDtinfoList)
			throws Exception {
		try {
			if(pubDtinfoList.size()>0)
			{
				 for(PubDtInfo pubDtInfo:pubDtinfoList){
	...
	public int claimAndReceiveCount(SysUserDto sysUser) throws Exception{
		...
		if(sysUser.getUserType().equals("1") && StringUtil.isNotEmpty(sysUser.getDeptCode())){ //警示人员登录：
			...
			checkDep = sysUser.getSysDepart().getAdcode(); //当前用户的行政编码即为审批部门编码
			userType = "2";
		}
	 ...
	 */
	@Override
	public List<PubDtInfoDto> queryPubdtinfoCancelListJSON(PageRequest request)
			throws Exception {
		try {
			Map<String, Object> searchMap=  request.getParams();
	 ...
	 */
	@Override
	public List<PubDtInfoDto> selectTransferPubdtinfoList(String checkDep,
