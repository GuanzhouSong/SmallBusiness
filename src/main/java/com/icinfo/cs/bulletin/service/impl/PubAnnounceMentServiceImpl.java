...
public class PubAnnounceMentServiceImpl extends MyBatisServiceSupport implements IPubAnnounceMentService {
	...
	public ModelAndView doGetViewByPubType(String UID, String pubType, ModelAndView view) throws Exception {
		...
		if (pubType.equals(BulletinType.AnomalyIn.getCode()) 
				|| pubType.equals(BulletinType.AnomalyRemind.getCode())) {
		...
		if (pubType.equals(BulletinType.AnomalyOut.getCode())) {
			...
			PubOpanoMaly pubOpanoMalyout = pubOpanoMalyService.selectPubOpanoMalyByBusExcList(UID);
			view.addObject("pubOpanoMalyout", pubOpanoMalyout);
			view.addObject("pubOpaDetail", pubOpaDetail);
		}
		// 4.抽查检查公告
		if (pubType.equals(BulletinType.SpotBulletin.getCode()) 
				|| pubType.equals(BulletinType.CheckBulletin.getCode())
			...
			if(pubType.equals(BulletinType.SpotTaskBulletin.getCode())){
				List<PubScPlanTaskDto> dtos = pubScPlanTaskMapper.selectScentTaskForBulletin(UID);
				for(PubScPlanTaskDto pubScPlanTaskDto : dtos){
					List<PubSccheckItem> pubScCheckItemList = pubScCheckItemService.selectPubSccheckItemByTaskUid(UID);
					String itemNames = "";
			...
			PubAnnounceMent pubAnnounceMent = doGetpubAnnounceMentInfo(UID, null);
	...
	public void doCreatBulletinAllInfo() throws Exception {
		try {
			...
			int count = Justicetotal + punishTotal + opanoMalyTotal + SoptCheckTotal + expCrimeTotal
					+ erEscAppinfoTotal;
