		            ...
		            if (null != unReportList && unReportList.size () > 0) { 
		                for ( PubScentDto pubScentDtoDto : unReportList ) { 
		                	try {
			                	...
			                	String entList= JSONArray.fromObject(entNameArr).toString(); 
			                	if(entCount!=null&&entCount>0){
			                		//如果没有绑定联连则不推送消息
				                	if(checkTel(linatel)){
				                		    String content="您担任联络员的"+entName+"等"+entCount+"家企业在"+randomDate+"已被浙江省工商局抽中为"+taskName+"工作的抽查检查对象。";
						                    String json = "{\"title\" : \"抽查检查提醒\", " + "\"pusher\" : \"国家企业信用信息公示系统\", " + "\"receiver\" : \""+linatel+"\"," + "\"sourceUrl\" : \""+url+"/sccheck/wechat/toMessageSccheckView?tel="+linatel+"&sendDate="+DateUtil.getDate ("yyyyMMdd")+"\"," + "\"type\" : \"3\"," + "\"subType\" : \"yr_random_check\"," + "\"content\" : \""+content+"\"," + "\"additionInfo\" : {" + "\"toDo\" : \"请积极配合做好相关准备\"," + "\"basicInfo\" : \"抽查检查提醒\","+ "\"sendTime\" : \""+DateUtil.getDate ("HH:mm")+"\"," +"\"remark\" : \"客服电话 4008884636\"},"+ "\"pushChannels\":\"1,2\","+"\"entList\":"+entList.toString()+""+"}";
